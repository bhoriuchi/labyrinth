...
module.exports = function (env) {
	...
	return function(opts) {
		...
		return function(id, status, logs, fetchOpts) {
			...
			return factory.transaction(function(t) {
				...
				.transaction(t)
				.view(['started', 'workflow.id', 'workflow.started', 'step.id', 'step.waitOnSuccess', 'step.success', 'step.fail', 'step.exception'])
				...
				.then(function(run) {
					...
					return factory.time({transacting: t}).then(function(time) {
						...
						// emit a step completion
						dream.mods.io.to(run.workflow.id).emit(_SYS.sockets.message, {
