	...
	return function(opts) {
		...
		return function(req, res, next) {
			...
			return factory.transaction(function(t) {
				...
				.transaction(t)
				.view(['status', 'workflowVersion', 'override'])
				...
				.then(function(run) {
					...
					if (run.override) {
						try {
							fetchOpts.override = JSON.parse(run.override);
						}
						catch (err) {
							console.log('error parsing override');
						}
					...
					.transaction(t)
					.view('resume')
					.getResource(req.params.id, _.cloneDeep(fetchOpts))
					.end()
					.then(function(run) {
